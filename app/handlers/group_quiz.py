...
async def quiz_end(state: FSMContext, user_name: Optional[str], bot: Bot) -> None:
    """
    ...
    try:
        ...
        await state.clear()
        quiz_message_id = quiz_data.get("quiz_message_id")
        quiz_chat_id = quiz_data.get("quiz_chat_id")
        if quiz_message_id:
            if user_name:
                await bot.edit_message_media(
                    media=input_media_animation.InputMediaAnimation(
                        ...
                        caption=Messages.quiz_finish_win_message.format(
...
async def quiz_set_state(
    state: FSMContext, fumo: Fumo, quiz_message_id: int, quiz_chat_id=int
) -> None:
    """
    ...
    await state.update_data(quiz_message_id=quiz_message_id)
    await state.update_data(quiz_chat_id=quiz_chat_id)
...
async def quiz_start(
    ...
    if fumo:
        await quiz_end(state, user_name=None, bot=bot)
        quiz_message = await bot.send_photo(
            chat_id=chat_id, photo=fumo.file_id, caption=Messages.quiz_guess_message
        )
        await quiz_set_state(state, fumo, quiz_message.message_id, quiz_message.chat.id)
    await session.close()
...
async def cmd_quiz(
    ...
    if message.from_user.id == Config.ADMIN_CHAT_ID:
        ...
        if fumo:
            ...
            await quiz_set_state(
                state, fumo, quiz_message.message_id, quiz_message.chat.id
            )


@router.message(MessageFromChannelFilter())
@media_group_decorator(only_album=False)
async def random_quiz_for_channel(
    messages: List[types.Message], session: AsyncSession, state: FSMContext, bot: Bot
) -> None:
    """
...
async def cmd_quiz_guess(
    ...
    if message.reply_to_message.message_id == quiz_message_id:
        if (
            message.text.lower() == fumo_name.lower()
            or message.text.lower() == fumo_name.split()[0].lower()
        ):
            ...
            await message.reply(
                Messages.quiz_success_message.format(
                    fumo=f"{fumo_name} {fumo_link if fumo_link else ''}".strip()
                ),
                link_preview_options=LinkPreviewOptions(is_disabled=True),
            )
            await quiz_end(state, user_name=message.from_user.full_name, bot=bot)
        else:
...
async def cmd_my_fumos(message: types.Message, session: AsyncSession) -> None:
    result = await db_quiz_get_records_for_user_id(
        session, user_id=message.from_user.id, group_id=message.chat.id
    )
    if len(result) > 0:
        fumo_names_text = "\n".join(
            [
                f"[{escape_markdown(row.fumo_name)}]({row.fumo_link}) \\- {row.fumo_count}"
                for row in result
            ]
        )
        await message.reply(
...
async def cmd_leaderboard(message: types.Message, session: AsyncSession) -> None:
    ...
    if len(result) > 0:
        ...
        fumo_names_text += "\n".join(
            [f"{row.user_name} - {str(row.fumo_count)}" for row in result]
        )
